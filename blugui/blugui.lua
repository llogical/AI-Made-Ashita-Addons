...
ashita.events.register('d3d_present', 'blugui_present', function()
    ...
    if imgui.SetNextWindowSize then
        imgui.SetNextWindowSize(v2(state.ui.width, 0), ImGuiCond_FirstUseEver)
    end
   ...
   local open = { state.is_open }
    local begin_ok = imgui.Begin('BLU Spells##blugui_root', open, ImGuiWindowFlags_NoResize)
    ...
    if begin_ok then
        ...
        elseif state.apply_status == 'applying' then
            ...
            TextColored({0.60,0.60,0.60,1},'Not Set')
        end
        imgui.SameLine()
